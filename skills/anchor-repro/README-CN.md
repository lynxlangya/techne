...
`anchor-repro` 是 techne 的 bug 修复 skill。它强制 AI 协作者在改代码前先记录一次失败复现，再用同一个 probe identity 验证修复结果。
...
如果你明确只想安装 `anchor-repro` 这一个 skill，再使用单 skill 命令：
...
在目标项目里打开 agent，然后明确要求使用 `anchor-repro`。
...
```text
使用 anchor-repro skill。修复这个 bug：login test 失败并出现 "TypeError:
cannot read properties of undefined"。先复现，再用同一个 probe 验证修复。
```
...
`anchor-repro` 会把 ledger 写到目标项目里：
