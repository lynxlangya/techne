...
- `repro`：修 bug 前强制先复现，记录失败 probe，再用同一个 probe 验证修复。详细使用方式见 [skills/repro/README-CN.md](skills/repro/README-CN.md)。
- `vet`：做代码 review 前强制锚定真实 git diff，核算影响半径、核对 claim / finding 证据，再通过门控给出 approve / request-changes / blocked verdict。详细使用方式见 [skills/vet/README-CN.md](skills/vet/README-CN.md)。
- `intake`：开工前审问书面工程实现 brief，按固定 rubric 逐项核对，暴露 gap / solution-as-goal / contradiction / question，再输出 intent-level plan。详细使用方式见 [skills/intake/README-CN.md](skills/intake/README-CN.md)。
